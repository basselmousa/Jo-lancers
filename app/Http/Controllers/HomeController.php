...
//        dd($providers[0]->provider->skill[0]->skill);
    ...
    public function categoryProviders(Request $request, ServiceCategory $category)
    {
        ...
        $providers = ServiceProvider::whereIn("id", $typesProviders)
            ->whereNotNull("price_for_hour")
            ->get();
        DB::enableQueryLog();
        if ($request->sort == 'rate') {
//            DB::enableQueryLog();
            $providers = ServiceProvider::with(["skill.skill", "currency"])
                ->leftJoin("rates", "service_providers.id", "=", "rates.service_provider_id")
                ->join("service_provider_types" ,"service_provider_types.service_provider_id","=","service_providers.id")
               // ->join("service_providers", "service_providers.id", "=", "rates.service_provider_id")
                ->select(["service_providers.id as serId","service_providers.*", "rates.service_provider_id", DB::raw("avg(rate) as rate")])
                ->whereNotNull("service_providers.price_for_hour")
                ->whereIn("service_provider_types.service_type_id", $types)
                ->groupBy("rates.service_provider_id")
                ->orderBy("rate", "desc")
                ->get();
//            dd($providers);
...
//            DB::enableQueryLog();
            $providers = ServiceProvider::with(["skill.skill", "currency"])
                ...
                ->select(["service_providers.id as serId","service_providers.*", "rates.service_provider_id",DB::raw("avg(rate) as rate")])
                ->whereNotNull("service_providers.price_for_hour")
                //->whereIn("service_providers.id", $typesProviders)
                ->whereIn("service_provider_types.service_type_id", $types)
                ->groupBy("rates.service_provider_id")
                ->orderBy("service_providers.full_name", "asc")
                ->get();
//            dd($providers);
        }
        ...
        return view("hourly_providers", compact("providers", 'category','sort'));
    }
...
//        return view()
    ...
    public function searchByName(Request $request,ServiceCategory $category)
    {
        $providers = ServiceProvider::with(["skill.skill", "currency"])
            ->leftJoin("rates", "service_providers.id", "=", "rates.service_provider_id")
            ->join("service_provider_types" ,"service_provider_types.service_provider_id","=","service_providers.id")
            ->select(["service_providers.id as serId","service_providers.*", "rates.service_provider_id",DB::raw("avg(rate) as rate")])->whereNotNull("service_providers.price_for_hour")
